...
def get_queues(token: str) -> list[Queue]:
    ...
    if role not in manipulation_permission:
        ...
        raise HTTPException(status_code=403, detail="You don't have permission to get all queues")
...
def get_queue(token:str, queue_id:str) -> Queue:
    ...
    if role not in manipulation_permission:
        ...
        raise HTTPException(status_code=403, detail="You don't have permission to get a queue")
...
def get_queue_size(token: str, queue_id: str) -> str:
    ...
    if role not in manipulation_permission:
        ...
        raise HTTPException(status_code=403, detail="You don't have permission to get a queue size")
    # Check if the queue exists
    queue = search_queue(queue_id)
    if not queue:
